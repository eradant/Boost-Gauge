...
#Map input pressure to output range (ex. 0-5v to 0-45psi)
...
#Placeholder variable for calculating high score
max_value = 0
...
while True:
    ...
    if true_pressure >= 18.9:
        ...
        h_text.text = " "
        gc.collect()
    if true_pressure >= 20.8:
        ha_text.text = ">"
    else:
        ha_text.text = " "
        gc.collect()
    if true_pressure >= 23.5:
        hb_text.text = ">"
    else:
        hb_text.text = " "
        gc.collect()
    z_text.text = "{:.2f}".format(true_pressure)  
    x_text.text = "PSI"
    ...
    #boost = (chan.voltage * 9.48901) - bmp.pressure * 0.0145038
    #("{:>5.2f}".format((chan.voltage * 9.48901) - bmp.pressure * .0145038))
